...
to setup-landPrice
  ask patches [set landPrice 10000]
end
...
;     # Check neighbouring plots with higher quality
      [let interestingPlots ifelse-value inRadius? [patch-set (patches in-radius radius with [wineQ > myLowQuality and owner = 0])]
                                             [patch-set ([neighbors] of myPlots with [wineQ > myLowQuality and owner = 0])] ; [neighbors] of myPlots or patches in-radius x? include distance from farmstead in cost calculation
...
;         # SELL PLOT with the lowest quality
          [ let chosenPlot min-n-of 1 myPlots [wineQ]
...
to giBoardAction
  ask patches [ifelse wineQ < qualityStandard [set giLabel 0] [set giLabel 1]]    ; For now it doesn't make any sense, and it doesn't interact with Farms
  refreshElev
