...
The main exogenous factors driving the model are **sigma** (defining spatial heterogeneity), **climateScenario** (defining the speed of warming trend), and **votingMechanism** (allowing farms to engage in the endogenous institutional change process).

Several parameters significantly impact the outcome. (e.g., memory, qualityDelta and prec, which respectively indicate: how far the farm can look in the past to make decision, how much the quality standard is change in each successful emendation, and how precise are farms capturing quality changes in the previous years).
...
- Possibility of comunication between farms and creation of lobby groups
...
- Using the model in a specific case study with real gis data.

## NETLOGO FEATURES

R extension used to generate random raster with a Gaussian smooth, modified the GIS code example to calculate slope via convolution to attain slope in %.
