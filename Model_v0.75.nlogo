...
to setup-quality
  ...
  [
    ...
    set wineQ (((1 - climateW) * soilQ) + climateW * (1 - ( ((optGStemp - microclimate) ^ 2) / optGStemp)))
  ]
end
...
;   # the farm will also set fallow the plots that are not profitable (i.e., when the profit is lower that the fixed cost to keep it fallow , €2500)
    ask myProdPlots with [((wineYield * giWinePrice) - fixCost - varCost) <= -2500] [print "fallowPool" set fallow 1 set pcolor black]
...
; # Is my quality decreasing in the last three years? I need to find new plots high quality plots!
  ...
  [
    let meanPV mean (sublist pastVintages (ticks - memory) (ticks))
 ...
 ;  # Only if there are interesting plots to buy makes sense to sell plots.
      ...
      [patch-set (patches in-radius radius with [wineQ > qualityStandard and owner = nobody and (member? [who] of myself pastOwners) = false and prof > 0])]    ; They know if a plot is profitable, thus they avoid the slopy plots
      [patch-set ([neighbors] of myPlots with [wineQ > qualityStandard and owner = nobody and (member? [who] of myself pastOwners) = false and prof > 0])]      ; [neighbors] of myPlots or patches in-radius x? include distance from farmstead in cost calculation
...
; # Check neighbouring plots with quality higher than standard and no owner
  let interestingPlots ifelse-value inRadius?
  [patch-set (patches in-radius radius with [wineQ > qualityStandard and owner = nobody and (member? [who] of myself pastOwners) = false and prof > 0])]    ; They know if a plot is profitable, thus they avoid the slopy plots
  [patch-set ([neighbors] of myPlots with [wineQ > [myQuality] of myself and owner = nobody and (member? [who] of myself pastOwners) = false and prof > 0])]      ; [neighbors] of myPlots or patches in-radius x? include distance from farmstead in cost calculation
...
to abandonFarming
  ...
  [
    let signProfit reduce * (sublist pastProfits (ticks - memory) (ticks))
    if signProfit < 0  ; if three years of negative profits
    [
      if  (myPlots with [farmStead = 0]) = no-patches
      [
        set bankruptcies bankruptcies + 1
        set plantingRights plantingRights + 1
        ask patch-here
...
to expandFarms    ; what about price of planting rights?
  ...
  set globalPlantingRights globalPlantingRights - (count topFarms with [capital > 100000])

  ask topFarms with [capital > 100000]
  [
    set capital capital - plantingRightsPrice
    set plantingRights plantingRights + 1
  ...
  [patch-set ([neighbors] of myPlots with [wineQ > [myQuality] of myself and owner = nobody and (member? [who] of myself pastOwners) = false ])]      ; [neighbors] of myPlots or patches in-radius x? include distance from farmstead in cost calculation
...
to qualityControl
  ask giBoards
  [
  set capital capital - ((1 - delta) * giFees)   ; Suppose they make no profit, all is spent in quality controls and all the rest invested in Marketing / collective brand value
  ]
end
...
; # Each Farm vote
  ...
  [
    ...
    [
      if length pastVintages > memory
      [
        let meanPV precision mean (sublist pastVintages (ticks - memory) (ticks)) prec   ; looking back of x years in pastVintages (driven by parameter memory).
