...
to setup-climate
  ...
  ask patches [
    set microclimate GSavgTemp  ; add increase dep on elevation.
  ]
end
...
to setup-raster
  ...
  set minelev [] set minelev MinimumElevation
  set maxelev [] set maxelev MaximumElevation
  r:put "s" sgk
  r:put "Min" minelev
  r:put "Max" maxelev
...
to setup-world
  ...
  let min-elevation gis:minimum-of elevation-data
  let max-elevation gis:maximum-of elevation-data
  ask patches
  [
    if (elevation <= 0) or (elevation >= 0)
    [set pcolor palette:scale-scheme  "Sequential"  "YlOrBr" 9 elevation min-elevation max-elevation                               ; Easiest way with fixed sequential scale
     ; let green-netlogo extract-rgb green                                                                                           ; MANUAL Three COLOURS with netlogo col
     ; let brown-netlogo extract-rgb brown
     ; let yellow-netlogo extract-rgb yellow
     ; set pcolor palette:scale-gradient ( list green-netlogo yellow-netlogo brown-netlogo ) elevation min-elevation max-elevation
     ; set pcolor palette:scale-gradient [ [0 109 44] [254 224 139] [84 48 5] ] elevation min-elevation max-elevation                 ; Manual three coulour with RGB
    ]
  ]
end
...
to climateChange
  ;ifelse ClimateScenario is ParisAgreement [] []
