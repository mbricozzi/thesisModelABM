...
to farmersHeuristic
  ask farms [
;    ifelse heuristicLoop? [loopHeuristic] [heuristic]
    heuristic
  ]
end
...
;      let totalCost sum [fixCost] of myPlots + sum [varCost] of myPlots with [ageVines > 3]
;
...
;          ifelse capital >= (first ([landPrice] of interestingPlots with-min [landPrice])) + totalCost + installCost ; must also count the maintenance cost of the plot you want to buy (there should be an installation cost)
;
...
;                                  ifelse myquality > qualitystandard [print "high"][print "low"]
;                                  show bankruptcies
;                                  ask myPlots [
;                                                set owner nobody
;                                                set pastOwners insert-item (length pastOwners) pastOwners ([who] of myself)
...
;
;
;        let totalRevenue ( giWinePrice * (wineYield * count myPlots with [ageVines >= 3]) )
...
;;         # BUY NEW PLOT with higher quality and lowest price
;          [
;            ; If you want to implant vines in a new plot you have to explant and sell an old one (with the lowest quality).
;            let lowQplot myPlots with [farmStead = 0] with-min [wineQ]
;            if any? lowQplot [
;             ask lowQplot [set owner nobody set pastOwners insert-item (length pastOwners) pastOwners ([who] of myself) set ageVines 0]
;             ask fTokens-on lowQplot [die]
;             set myPlots myPlots with [not member? self patch-set lowQplot]
;             set capital capital + first [landPrice] of lowQplot
;            ]
;            ; Now buy a new plot
;            let chosenPlot interestingPlots with-min [landPrice]
;            ask chosenPlot [set owner [who] of myself                                        ; ATTENTION HERE EACH FARM DOES THIS ONE AFTER THE OTHER, SOMEBODY WILL RANDOMLY BE SELECTED TO MOVE FIRST THEREFORE HAVING MORE PLOTS TO CHOOSE FROM.
...
;
;;         1.1.2) The farmer has not enough capital to buy
;;         # SELL PLOT with the lowest quality if it helps to get back above the quality standard  (otherwise even farms that were doing really well the sell all plots and die if they have no other interesting plots to buy)
;          [
;            if  (myPlots with [farmStead = 0]) = no-patches [ask patch-here [set owner nobody
;                                                                             set pastOwners insert-item (length pastOwners) pastOwners ([who] of myself)]
;                                                                             ask fTokens-on patch-here [die]
...
;            let chosenPlot myPlots with [farmStead = 0] with-min [wineQ]
;
;            let newMyPlots myPlots with [not member? self patch-set chosenPlot]
;            if mean [wineQ] of newMyPlots > qualityStandard
;            [
;            ask chosenPlot [set owner nobody set pastOwners insert-item (length pastOwners) pastOwners ([who] of myself) set ageVines 0]
;            ask fTokens-on chosenPlot [die]
;            set myPlots newMyPlots
;            set capital capital + first [landPrice] of chosenPlot
;            ]
;          ]
;        ]
;;     1.2) There are no interesting plots
;;       # SELL PLOT with the lowest quality if it helps to get back above the quality standard (same as before)
;        [
;          if  (myPlots with [farmStead = 0]) = no-patches [ask patch-here [set owner nobody
;                                                                           set pastOwners insert-item (length pastOwners) pastOwners ([who] of myself)]
;                                                                           ask fTokens-on patch-here [die]
